...
EBE_pollution <- madrid %>% 
  ...
  geom_point()+
  labs(y = "Ethylbenzene (μg/m³)", x = "Date")
...
madrid_pollution <- madrid %>% 
  group_by(mnth, year) %>% 
  summarize(maximum = max(EBE, na.rm = TRUE)) %>% 
  ggplot(aes(x = mnth, y = maximum))+
  geom_col()+
  facet_wrap(~year)+
  coord_flip()
...
pollution_diff <- difference %>% 
  pivot_longer(cols = c("BEN":"TOL"), values_to = "difference")
