...
GP <- read.csv("GP.csv")
library(tidyverse)
joint_data <- inner_join(GP, IM)

joint_data$Condition <- factor(joint_data$Condition)
joint_data$Gender <- factor(joint_data$Gender)
```
...
```{r exercise 02}
x_id <- 1:10
x_value <- rnorm(10, mean = 180, sd = 50)
x_df <- data.frame(x_id, x_value)

y_id <-  1:20
y_value <- rnorm(20, mean = 10, sd = 2)
